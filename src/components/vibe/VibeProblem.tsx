  ...
  return <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-16">
          {tools.map((tool, index) => <div key={index} className="group bg-white rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 hover:border-red-200">
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 bg-gradient-to-br ${tool.color} rounded-xl flex items-center justify-center overflow-hidden`}>
                  <img src={tool.logo} alt={`${tool.name} logo`} onError={e => {
              ...
              <ul className="space-y-2">
                {tool.issues.map((issue, issueIndex) => <li key={issueIndex} className="flex items-start gap-2 text-xs text-gray-600">
