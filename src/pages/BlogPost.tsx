...
const BlogPost = () => {
  ...
  const renderContent = () => {
    ...
    if (typeof post.content === 'string') {
      return <div className="prose prose-lg max-w-none" dangerouslySetInnerHTML={{
        __html: post.content
      }} />;
    }
      ...
      if (Array.isArray(post.content)) {
        return <div className="space-y-6">
            ...
            if (block._type === 'block') {
              ...
              return <p key={index} className="text-lg leading-relaxed text-gray-700 mb-4">
                    {text}
                  </p>;
            }
            return null;
      ...
      <main className="pt-24 py-0">
        <div className="bg-white border-b">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <Link to="/blog" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 transition-colors duration-200">
        ...
        <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <header className="mb-12">
            <div className="flex flex-wrap gap-2 mb-6">
              {post.categories?.map(category => <Badge key={category} className="bg-primary text-white hover:bg-primary/90">
                  {category}
                </Badge>)}
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-8 border-b border-gray-200">
              <div className="flex items-center gap-6 text-gray-600">
                ...
                <div className="flex items-center gap-4 text-sm">
                  {post.published_at && <div className="flex items-center gap-1">
          ...
          {post.featured_image && <div className="mb-12">
              <img src={post.featured_image} alt={post.title} className="w-full h-96 object-cover rounded-2xl shadow-lg" />
            </div>}
          ...
          {post.tags && post.tags.length > 0 && <div className="mt-12 pt-8 border-t border-gray-200">
              ...
              <div className="flex flex-wrap gap-2">
                {post.tags.map(tag => <Badge key={tag} variant="outline" className="text-gray-600 hover:text-primary hover:border-primary">
                    #{tag}
                  </Badge>)}
              </div>
            </div>}
